...
public class FootPath_Preview extends Activity {
    ...
    ImageView imgvd, imgvd2,img_logo;
...
/**************************************************************************************************************************************/
        ...
        unitCode = MainActivity.uintCode;
        unitName = "";
            ...
            try {
                ...
                for (String pntName : FootPath_Vendor.pointNameMap.keySet()) {
                    if (pntName.trim().equals(pointName)) {
                        pointCD="";
                        pointCD = FootPath_Vendor.pointNameMap.get(pntName);
                        break;
                    }
                }
        ...
        protected void onPostExecute(String result) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    // TODO Auto-generated method stub
                    try {
                        if (null != WebService.Challan_response) {
                            if (WebService.Challan_response.equals("0^NA^NA")) {
                                showToast("Challan Generation Failed");
                            } else if (WebService.Challan_response.equals("2^NA^NA")) {
                                showToast("Already Challan Generated!");
                            } else if (WebService.Challan_response.equals("3^NA^NA")) {
                                showToast("Contact E-Challan Team!");
                            } else {
                        ...
                        e.printStackTrace();
                        showToast("Please check internet and try again!");
                    }
                }
            });
        }
